...
> **macOS 原生离线语音输入工具**  
> 100% 本地离线 · 阿里 SenseVoice 极速识别 · 标点原生输出不改一字 · 遵循 Apple HIG 黑曜石毛玻璃美学
...
- **标点原生输出，全文零改写**：逗号、句号、问号由识别模型在识别时一并产出，**不存在第二个语言模型二次改写你的原话**，口述内容逐字还原。
- **纯正 Apple HIG 原生体验**：
...
~/.voicetype/models/           首次启动时下载的本地模型（约 0.9GB）
```
...
1. 首次打开会弹出配置窗口，自动下载本地 AI 引擎（约 500MB）与语音模型（约 900MB），仅需一次；
