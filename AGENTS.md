...
- **100% 离线与隐私至上**：所有音频采集、重采样与语音识别（SenseVoice）必须且只能在本地芯片运行，严禁任何数据上传云端。
- **纯正 Apple HIG 规范**：
...
### 2.3 标点与文本保真
- 标点**只能来自识别模型本身**（SenseVoice 在识别时一并产出），不要再挂任何语言模型做“标点恢复”。
- 历史教训（已实测）：曾用 Qwen2.5-0.5B 加标点，结果：
  - **67.7%** 的输出因改写原文被保真校验丢弃，用户根本拿不到它的结果；
  - 它会把英文**翻译**成中文（`we need to finalize...` → `我们将在下个月完成定价。`），
    把中英混说里的英文词换掉（`feature`→`功能`、`merge`→`合并`）；
  - 字符级校验会剔除空格，导致它插入空格也能“通过”，产出
    `明天上午 10 点之前 把 初稿 发到 群里 。` 这类垃圾结果。
  - 详见 `docs/标点方案.md`。
- 任何环节都绝对禁止润色、翻译或篡改用户的原始口述内容。
...
- **优先使用 Apple Silicon 的 Metal GPU（`mps`）**，不可用时回退 `cpu`。
  实测（M2）启用 MPS 后：SenseVoice 提速约 9 倍，端到端从约 2 秒降到约 0.5 秒。
...
- **端到端耗时主要在语音识别这一步**（标点已由识别模型一并产出，不额外耗时）。
